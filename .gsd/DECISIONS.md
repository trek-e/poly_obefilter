...
- "Exponent accumulation pattern — cutoffCV * cvAmount + voct computed in log domain, then single pow(2, ...) call. Avoids multiplied separate pow calls and keeps pitch-domain math correct"
- "Purple (#8040a0) dot indicators for modulation inputs (FM, V/Oct), distinct from blue (#3060a0) for audio-path inputs — visual grouping by function"
- "V/Oct label rendered as 'V/O' — compact form fits 14 HP panel without clipping"
