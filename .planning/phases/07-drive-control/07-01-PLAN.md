...
<task type="auto">
  ...
  <files>src/CipherOB.cpp, res/CipherOB.svg</files>
  <action>
...
4. In CipherOBWidget constructor, add components:
   ...
   - Add CV input: createInputCentered<PJ301MPort>(mm2px(Vec(55.0, 50.0)), module, CipherOB::DRIVE_CV_INPUT)
   - Add attenuverter: createParamCentered<RoundSmallBlackKnob>(mm2px(Vec(55.0, 62.0)), module, CipherOB::DRIVE_ATTEN_PARAM)
...
1. Add parameter smoother in SVFilter struct (or use local smoothing in process):
   Add at module level (in CipherOB struct, after filters array):
   ```cpp
