...
4. In widget constructor (after line 111 RESONANCE_PARAM widget), add attenuverter widget:
   ...
   addParam(createParamCentered<RoundSmallBlackKnob>(mm2px(Vec(35.56, 98.0)),
       module, CipherOB::RESONANCE_ATTEN_PARAM));
   ```
...
Grep for RESONANCE_ATTEN_PARAM in CipherOB.cpp - should appear 4 times (enum, configParam, getValue, widget)
  </verify>
  <done>
...
- Position cy=98 matches the widget position in CipherOB.cpp
...
The components group has display:none so this is purely a development reference marker, not visible in the final panel.
  ...
  <verify>
Open res/CipherOB.svg in a text editor and verify the resonance-atten-knob circle element exists with cx="35.56" cy="98"
  </verify>
  <done>
...
Run: `grep -c "RESONANCE_ATTEN_PARAM" src/CipherOB.cpp` - should return 4
  </verify>
  <done>
