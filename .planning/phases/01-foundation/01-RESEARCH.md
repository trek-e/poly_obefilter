...
Critical constraints include: plugin major version must match Rack major version (currently 2.x), all SVG text must be converted to paths, plugin.json slug cannot change post-release, and the component library is CC BY-NC 4.0 (noncommercial use only). The PHANTOM · 8 reference project demonstrates the exact visual style and branding to match.
...
│   └── CipherOB.cpp  # Module implementation (PascalCase)
├── res/                 # Resources directory
│   └── CipherOB.svg  # Panel design (matches module slug)
...
**Example:**
```makefile
# Source: https://github.com/trek-e/COLOSSUS · 16/blob/main/Makefile
RACK_DIR ?= ../Rack-SDK
...
**When to use:** Modules supporting polyphony (8-voice for COLOSSUS · 16)
**Example:**
```cpp
// Source: https://vcvrack.com/manual/PluginGuide
void process(const ProcessArgs& args) override {
...
struct CipherOBWidget : ModuleWidget {
    CipherOBWidget(CipherOB* module) {
        ...
        addParam(createParamCentered<RoundBlackKnob>(mm2px(Vec(30.0, 40.0)), module, CipherOB::CUTOFF_PARAM));

        // Audio input (top left convention)
        addInput(createInputCentered<PJ301MPort>(mm2px(Vec(10.0, 20.0)), module, CipherOB::AUDIO_INPUT));

        // Filter outputs section
        addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(50.0, 80.0)), module, CipherOB::LP_OUTPUT));
        addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(50.0, 95.0)), module, CipherOB::HP_OUTPUT));
    }
};
```
...
- [COLOSSUS · 16 GitHub Repository](https://github.com/trek-e/COLOSSUS · 16) - Reference project structure and plugin.json
