...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/CipherOB.cpp"
      ...
      exports: ["CipherOB", "CipherOBWidget"]
    - path: "README.md"
  ...
  key_links:
    - from: "plugin.json"
      ...
      pattern: '"slug".*"CipherOB"'
    - from: "res/CipherOB.svg"
      to: "src/CipherOB.cpp"
...
2. Run helper.py to create plugin structure:
   ```bash
   $RACK_DIR/helper.py createplugin CipherOB
   ```
...
3. Update plugin.json with correct metadata:
   ...
   - sourceUrl: "https://github.com/trek-e/CipherOB"
...
<task type="auto">
  ...
  <action>
Create panel design in Inkscape matching PHANTOM · 8 visual style:
...
3. Design panel visual style matching PHANTOM · 8:
...
6. Finalize SVG:
   ...
   - Save As: res/CipherOB.svg

**Visual style reference:** PHANTOM · 8 at https://github.com/trek-e/COLOSSUS · 16 uses same color palette, font hierarchy, and dark industrial aesthetic. Match that style exactly.
...
- Color codes must be exact for helper.py: red=#ff0000, green=#00ff00, blue=#0000ff, magenta=#ff00ff (Pattern 2)
  ...
  <verify>
1. Open res/CipherOB.svg in Inkscape
...
6. Visual style matches requirements (dark background #1a1a2e, title visible, sections labeled)
  ...
  <done>
Panel SVG exists at res/CipherOB.svg with correct dimensions, component placeholders on hidden layer, PHANTOM · 8 visual style, and all text converted to paths.
  </done>
</task>
...
1. Generate module code from SVG:
   ...
   $RACK_DIR/helper.py createmodule CipherOB res/CipherOB.svg src/CipherOB.cpp
   ```
   This auto-generates:
...
2. Review generated src/CipherOB.cpp:
...
5. Compile plugin:
   ...
   make
   ```
   This produces dist/CipherOB.vcvplugin
...
- Common pitfall: RACK_DIR not set (Pitfall 5)
  ...
  <verify>
1. `ls src/` shows CipherOB.cpp
2. `grep -c "createParamCentered" src/CipherOB.cpp` returns 4 (4 params)
3. `grep -c "createInputCentered" src/CipherOB.cpp` returns 3 (3 inputs)
4. `grep -c "createOutputCentered" src/CipherOB.cpp` returns 4 (4 outputs)
5. `cat plugin.json` shows CipherOB in modules array
6. `make` completes without errors
7. `ls dist/` shows .vcvplugin file
  </verify>
  <done>
Module code exists at src/CipherOB.cpp with widget positioning from SVG, plugin.json references module, plugin compiles successfully to .vcvplugin file.
  </done>
</task>
...
- Mention it's part of COLOSSUS · 16 series
...
- GPL-3.0 license matching PHANTOM · 8
</action>
  <verify>
...
- Panel design (CipherOB.svg) matching PHANTOM · 8 visual style
...
1. Open res/CipherOB.svg in Inkscape:
   ...
   - Visual style matches PHANTOM · 8 (dark background #1a1a2e, dusty orange outputs section)
...
4. Open VCV Rack:
   ...
   - CIPHER · OB should appear in browser
   - Drag module to rack
...
**Approval criteria:**
- Panel design matches PHANTOM · 8 visual style
...
**Code Verification:**
- [ ] plugin.json contains CipherOB module entry with correct tags
- [ ] src/CipherOB.cpp has configParam/configInput/configOutput calls
...
**Visual Verification:**
- [ ] Panel matches PHANTOM · 8 style (colors, fonts, layout)
...
5. Panel visual style matches PHANTOM · 8 exactly (color palette, fonts, layout)
