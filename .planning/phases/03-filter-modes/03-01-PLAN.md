...
wave: 1
depends_on: []
files_modified: [src/SVFilter.hpp, src/CipherOB.cpp]
autonomous: false
...
3. At the end of process(), compute all four outputs using Cytomic SVF equations with CLEAN (pre-saturation) v1_raw:

   CRITICAL: Use v1_raw (NOT v1) for output taps. Saturation is ONLY for feedback stability.
...
4. Update the NaN check to return zeroed struct: `return {0.f, 0.f, 0.f, 0.f};`
...
AVOID: Using saturated `v1` for outputs. This would compress dynamics and alter frequency response. The saturation stays in the feedback path (ic1eq update) ONLY.
  </action>
  ...
  <done>SVFilter::process() returns SVFilterOutputs struct with lowpass, highpass, bandpass (using v1_raw), and notch</done>
</task>
